...
export const Search = () => {
  ...
  const searchDB = () => {
    ...
    posts.forEach((element) => {
      ...
      if (search === 'Title') {
        if (element.post.title.toLowerCase().includes(
          searchVal.toLowerCase()
          )) {
          newPosts.push(element)
        }
      } else if (search === "Location") {
        if (element.post.location.toLowerCase().includes(
          searchVal.toLowerCase()
          )) {
          newPosts.push(element)
        }
      } else if (search === "Username") {
        if (element.post.username.toLowerCase().includes(
          searchVal.toLowerCase()
          )) {
          newPosts.push(element)
        }
      } else if (search === "Caption")  {
        if (element.post.caption.toLowerCase().includes(
          searchVal.toLowerCase()
          )) {
          newPosts.push(element)
        }
  ...
  return (
    <>
      {user ? <div className="search">
        ...
        { msg ? <h1 style={{textAlign: "center"}}>No Results</h1>
        : <div className="posts">
